...
@synthesize window, splitViewController, navigationController, notesListViewCtrl;
...
- (void)dealloc 
{
	...
	[[NSNotificationCenter defaultCenter] removeObserver:notesListViewCtrl];
    
    [noteContentViewCtrl release];
    [notesListViewCtrl release];
    [navigationController release];
    [noteNavigationController release];
	[window release];
...
{	
    ...
    notesListViewCtrl = [[NotesListViewController alloc] init];
	// create a navigation controller using the new controller
	navigationController = [[UINavigationController alloc] initWithRootViewController:notesListViewCtrl];
    noteContentViewCtrl = [[NoteViewController alloc] init];
    notesListViewCtrl.iNoteContentViewCtrl = noteContentViewCtrl;
    [[NSNotificationCenter defaultCenter] addObserver:notesListViewCtrl selector:@selector (needRefresh:)
												 name:NotesNeedRefresh object:noteContentViewCtrl];
    
	navigationController.navigationBar.barStyle = UIBarStyleBlackOpaque;
    noteNavigationController = [[UINavigationController alloc] initWithRootViewController:noteContentViewCtrl];
    
    // Create splitViewCtrl
	splitViewController = [[UISplitViewController alloc] init];
	splitViewController.viewControllers = [NSArray arrayWithObjects:navigationController, noteNavigationController, nil];
	splitViewController.delegate = noteContentViewCtrl;
