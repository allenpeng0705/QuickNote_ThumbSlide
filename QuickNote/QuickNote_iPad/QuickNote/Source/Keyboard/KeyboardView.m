...
- (void) changeKeyboard
{
    ...
    if (iKeyboard != currentKB) {
        iCurrTouch = nil;
        iKeyboard = currentKB;
        [iHandler setKeyboard:iKeyboard];
...
-(void)attachKeyboard:(Keyboard*)aKeyboard AndKeyboardOutputListener:(id<KeyboardOutputListener>)aListener
{
    ...
    iCurrTouch = nil;
    [iHandler release];
    iHandler = [[KeyboardHandler alloc] init];
    [iHandler setKeyboard:iKeyboard];
    [iHandler setKeyboardInteractionListener:self];
    [iHandler setKeyboardOutputListener:aListener];
    [iHandler setKeyboardBridge:self];
...
- (void)drawRect:(CGRect)clip
{
    ...
    if (iTracePointIndex >= 5) {
        ...
        CGContextSetLineWidth(iMainGraphicCtx, 5);
...
{
    return [iHostEditor shouldCapitalize];
}

-(BOOL)shouldSendBackspace
{
    return [iHostEditor shouldSendBackspace];
}

-(NSString*)stringAroundCursor
{
    return [iHostEditor stringAroundCursor];
}
...
-(void)showPreview:(Key*)aPressedKey WithSound:(BOOL)aFlag;
{
    ...
    rect.origin.y = adjustPosY - (3*adjustHeight)/2 - 20;
    rect.size.width = 3*adjustWidth/2;
    rect.size.height = adjustHeight*3/2;
...
{   
    [iHostEditor sendText:aText WithSpace:aSpaceFlag];
}

-(void)replaceTextAroundCursor:(NSString*)aText
{
    [iHostEditor replaceTextAroundCursor:aText];
}

-(void)selectWordAroundCursor
{
    NSRange range = [iHostEditor rangeAroundCursor];
    [iHostEditor setSelectedRange:range];
}

-(void)sendBackspace
{
    [iHostEditor handleBackspace];
}

-(void)sendEnter
{
    [iHostEditor handleEnter];
}
...
{	
    if ((self = [super initWithFrame:frame])) {
        ...
        if (UIInterfaceOrientationIsPortrait(orientation) == YES) {
            iWidthRatio = 76800.0/320.0;
            iHeightRatio = 30400.0/208.0;
        } else {
            iWidthRatio = 70300/320.0;
            iHeightRatio = 30400/208.0;
        }
        iShiftModeOn = NO;
        return self;
    }
