 ...
 */
//#define N_PRUNING_SAMPLE_POINTS 40
 ...
 * When (at the pruning stage) a point is compared
...
struct MemoryDataBuffer {
	/* This fixed pointer always points at the start of the memory buffer */
	char *block_begin;
...
/*
 * An encoding node holds one Unicode character (code point) and a link
 * that may point to another encoding node.
 */
struct EncodingNode {
...
static Point start_direction(int16 * signal, int32 n, int16 radius);
 ...
 *
 * *aFilter - a pointer to a created recognizer object
 *
...
void initFilter(Filter* aFilter, FILE* aFile, const FilterTable* aFilterTable, FilterParams* aFilterParms)
{
	...
	/* Ensure stream is past UTF-8 BOM, and get the stream position at that point */
 ...
 *
 * *str - a pointer to string, the string does not need to be NULL-terminated.
 * n - the number of characters in the input string, which will be part of the pattern's
 * header
 * *encoding_table - a pointer to the encoding table
 *
 ...
 * *encoding_table - pointer to the encoding table
 *
 * Returns the number of bytes required to encode the given string.
 */
static int32 get_encoding_byte_count(const char *str, int32 n, const EncodingTable *encoding_table)
 ...
 *
 * *str - a pointer to an UTF-8 string
 * n - the number of characters in the string
 * *pat - a pointer to the output buffer (where the encoded byte pattern
 * will be written)
 * ds - the data segment that will be part of the pattern
 * *encoding_table - the encoding table to use when encoding the pattern
 * *out_byte_count - a pointer to an integer. This integer will hold
 * the number of bytes written to the output buffer pointed to by *pat
 *
 * Returns status of completion.
 */
static InternalFilterResult encode_string(const char *str, int32 n, uint8 *pat, uint8 ds, const EncodingTable *encoding_table, int32 *out_byte_count)
 ...
 * properly decoded.
 * 
 * *in - a pointer to a pattern whose last encoded char should be found
 *
 ...
 *
 * *in - a pointer to the input pattern to decode
 * *out - a pointer to an output buffer to write out the decoded UTF-8 string to
 * *encoding_table - a pointer to the encoding table to use when decoding the pattern
 *
 */
 ...
 *
 * *in - a pointer to the input pattern to decode
 * *out - a pointer to an output buffer to write out the geometric pattern to
 * *encoding_table - a pointer to the encoding table to use when decoding the pattern
 *
 * Returns the number of points decoded into a geometric pattern.
 */
static int32 decode_points(const uint8 *in, int16 *out, const EncodingTable *encoding_table, const IndexIgnoreList *index_ignore_list)
{
 ...
 *
 * Returns a pointer to a new encoding table for the supplied set of key mappings.
 */
static EncodingTable * create_encoding_table(const FilterTable *key_mapping_set, const char *ignore_chars, int32 n_ignore_chars)
{
...
static void delete_encoding_table(EncodingTable *encoding_table)
{
	...
	for (i = 0; i < n; i++) {
		...
		while (root->next != NULL) {
			 ...
			 * and make the previous node point to what was the current
			 * node.
			 */
			 ...
			 */
			q_free(node);
			/* This also means that the previous node which pointed to the
			 * child should point to null.
			 */
			prev->next = NULL;
...
/*
 * Fills out the array pointed to by *index with all characters present
 ...
 *
 * *encoding_table - the encoding table to scan
 * *index - a pointer to the output buffer
 *
 */
static void create_flat_encoding_table_index(const EncodingTable *encoding_table, int32 *index)
{
 ...
 * indices_count - the number of entries (indices) pointed to by *index
 ...
 *
 * *encoding_table - a pointer to the encoding table
 * *stream - a pointer to the stream to read
 *
 */
static void calibrate_encoding_table(const EncodingTable *encoding_table, FILE *stream)
{
 ...
 *
 * *key_mapping_set - a mapping between strings and points
 */
static int32 get_unique_xy_pairs_count(const FilterTable *key_mapping_set)
{
	int32 i, j, n;
...
static fpos_t skip_past_utf8_byte_order_mark(FILE *stream)
{
	...
	if (utf8h1 == 0xef && utf8h2 == 0xbb && utf8h3 == 0xbf) {
		 ...
		 * Update the beginning-of-stream position variable so it points beyond
		 * the byte order mark.
		 */
		fgetpos(stream, &stream_pos);
	}
 ...
 * getline - a pointer to a function that can read a line from the *data_buffer
 *
 ...
 * 5. The entire block pointed to by filter->storage MUST be initialized to
 * zero.
 * 6. The *raw_stg parameter will be invalid when this function returns.
 *
 * *filter - the recognizer object
 * *raw_stg - a pointer to an unprocessed storage area
 */
static InternalFilterResult create_indices(uint8 *raw_stg, uint8 *blank_storage, int32 pattern_count, int32 char_mapping_count, Index **aIndices, int32 *index_count)
{
	int32 i, n;
	...
	*aIndices = indices;
	 ...
	 * the pointers would point to the raw (invalid!) storage area at step 3!
	 */
	stg = blank_storage;
	for (i = 0; i < n; i++) {
		/* Get the bucket size of this index */
		sz = buckets[i].size;
		/* Store the pointer to this storage position */
		if (sz == 0) {
			indices[i].ptr = NULL;
		}
		else {
	...
	for (i = 0; i < n; i++) {
		...
		/* Walk the pointer until a pattern with zero size is encountered. */
		while (*pat_ptr) {
...
static InternalFilterResult compute_index_values(Index *indices, int32 index_count, EncodingTable *encoding_table, IndexIgnoreList *index_ignore_list, EdgeKeyMap *edge_key_map, Parameters *parameters)
{
	...
	for (i = 0; i < n; i++) {
		...
		for (j = 0; j < m; j++) {
			...
			pat_len = decode_points(pattern_ptr, pattern, encoding_table, index_ignore_list);
...
/*
 * Sets a pattern's active bit in the data segment pointed to.
 *
 * *ds - a pointer to a data segment
...
/*
 * Sets a pattern's remove bit in the data segment pointed to.
 *
 * *ds - a pointer to a data segment
...
/*
 * Sets a pattern's reserved bit in the data segment pointed to.
 *
 * *ds - a pointer to a data segment
...
/* 
 * Converts an external input signal into an internal 2D-point pattern
 ...
 * max_len - the maximum number of points that can be written to the output buffer
 *
 * The return value is the number of sample points in the input signal. Note that
 * this number may be higher than the number of converted sample points written to
...
static void get_indices(Filter *aFilter, int16 *unknown, Index *index, int32 *index_n, float32 n_start_point_searched_radius, float32 n_end_point_searched_radius)
{
	...
	/* Search area (number of keys) around start point */
//	n = n_start_point_searched_keys;
	/* Search area (number of keys) around end point */
//	m = n_end_point_searched_keys;
	...
	find_encoded_neighbors(aFilter->encoding_table, aFilter->neighbors, unknown[0], unknown[1], start_nbors, n_start_point_searched_radius, &n);
	find_encoded_neighbors(aFilter->encoding_table, aFilter->neighbors, unknown[(aFilter->parameters->n_pruning_sample_points * 2) - 2], 
		unknown[(aFilter->parameters->n_pruning_sample_points * 2) - 1], end_nbors, n_end_point_searched_radius, &m);
...
FilterResults* filter(Filter* aFilter, const InputSignal* aInputSignal, const void* aReserved)
{
	FilterResults *rs;
...
//	int16 unknown[MAX_N_SAMPLE_POINTS * 2];
	Index *search_indices;
	...
	if (result_count > 1) {
		...
		if (!get_data_segment_active(*(reranked1[0].pat_ptr + 1))) {
			...
			if (ai != -1) {
				...
				/* Shift all results up to that point */
				for (i = 1; i < ai; i++) {
 ...
 *
 * *results - pointer to a sequence of Result structs
 * result_count - the number of Result structs pointed to
 ...
 *
 * *partition - pointer to a sequence of Result structs
 * partition_count - the number of Result structs pointed to
 * *reranked - pointer to destination buffer to hold the
 ...
 *
 * *segment - pointer to a sequence of Result structs
 * n - the number of result structs pointed to
 */
static void rerank_segment(Result *segment, int32 n)
{
...
static void get_directional_tendency(int16 *pattern, int32 n, uint8 *out_x, uint8 *out_y, Parameters *parameters, uint8 mode)
{
	...
	/* Keeps track of "change points" in X */
	x_chg_pt_counter = 0;
	/* Keeps track of "change points" in Y */
	y_chg_pt_counter = 0;
	if (mode == 1) {
...
static void prune_to_heap(Filter *aFilter, int16 *unknown, Index *search_indices, int32 n_search_indices)
{
	...
	BOOL is_qwerty;
	uint8 n_pruning_sample_points, n_sample_points;
	uint8 u_pec;
	int16 ul;
	int16 tsl;
	int16 lup;
	...
	n_pruning_sample_points = aFilter->parameters->n_pruning_sample_points;
	n_sample_points = aFilter->parameters->n_sample_points;
	is_qwerty = aFilter->kbd_type == QWERTY;
	ul = (int16)((float32)pattern_length(unknown, n_pruning_sample_points) / (float32)aFilter->parameters->key_width);
	get_string_trace(encoding_table, index_ignore_list, unknown, n_pruning_sample_points * 2, key_trace_buf, &n_key_trace_buf);
	...
	lup = max_side;
	ldown = min_side / 2;
	in_direct = start_direction(unknown, n_pruning_sample_points, (int16)(min_side / 2));

	for (i = 0; i < n_search_indices; i++) {
		/* Get the index */
		index = search_indices[i];
		/* Fetch pointer to first pattern addressed in index */
		pattern_ptr = index.ptr;
		if (pattern_ptr == NULL) {
			continue;
		}
...
//-------------------------------------------------------
			...
			pat_len = decode_points(pattern_ptr, candidate, encoding_table, index_ignore_list);
...
//			unfilter++;
			...
			//resample_noapprox(candidate, candidate_resampled, pat_len, n_pruning_sample_points);
			resample(candidate, candidate_resampled, pat_len, n_pruning_sample_points);
			d = shape_dist(unknown, candidate_resampled, n_pruning_sample_points * 2);
...
static void fuzzy_key_search(Parameters *parameters, uint8 *stg, int32 pattern_count, EncodingTable *encoding_table, IndexIgnoreList *index_ignore_list, int16 *unknown, Result *out_results, int32 *out_results_count)
{
	...
	match_count = 0;
	n_pruning_sample_points = parameters->n_pruning_sample_points;
	n_sample_points = parameters->n_sample_points;
	get_string_trace(encoding_table, index_ignore_list, unknown, n_sample_points * 2, key_trace_buf, &n_key_trace_buf);
	remove_string_trace_duplicates(key_trace_buf, n_key_trace_buf, stripped_key_trace_buf, &n_stripped_key_trace_buf);
	stg_ptr = stg;
...
//	int flag = 1;
	...
	n_sample_points = aFilter->parameters->n_pruning_sample_points;
	encoding_table = aFilter->encoding_table;
	index_ignore_list = aFilter->index_ignore_list;
	...
	get_corner_from_signal(aFilter, unknown, n_sample_points, in_corner, &in_corner_num);
	...
	for (i = 1; i < n; i++) {
		...
		/* Resample the candidate into an equidistant N-point pattern */
		resample_noapprox(candidate, candidate_resampled, pat_len, n_sample_points);
...
//		shape_location_dist(unknown, candidate_resampled, unk_center_x, unk_center_y, cand_center_x, cand_center_y, s, n_sample_points * 2, &sd, &ld);
		ld = distance_array(unknown, candidate_resampled, n_sample_points * 2) / n_sample_points;
 ...
 *
 * *pat is a sequence of (x,y) points
 * n is the number of points in *pat (this number must be even)
 *
...
/*
 * Returns the pointer to the pattern in the recognizer
 * that corresponds to the given string.
 *
 * *filter is a recognizer object
 * *str is a string
 *
 * The return value is a pointer to the pattern in the recognizer,
...
static BOOL add_extra_strings(Filter *aFilter, const char **strs, int32 strs_n)
{
	...
	/* Forward pointer to the end of the block where strings were decoded from extra
	 * storage */
...
static void resample(int16 *in, int16 *out, int32 n, int32 pts)
{
	...
	m = n * 2;
    /* Get the spatial length of the point sequence */
    ...
    x1 = in[0];
	y1 = in[1];
    /* The a variable keeps track of the interpolated point index */
    a = 0;
	out_max = pts * 2;
    for (i = 2; i < m; i+= 2) {
		x2 = in[i];
		y2 = in[i + 1];
		/* Number of points to interpolate into this segment */
		ptsseg = segment_buf[(i / 2) - 1];
		/* Find the dx and dy delta distances for each point to be
		* interpolated */
		dx = -1.0f;
		dy = -1.0f;
		if (ptsseg - 1 <= 0) {
		...
		/* Only try to interplate if at least one point should be interpolated
		 * into this segment */
		if (ptsseg > 0) {
			/* Iterate over all points to be interpolated into this segment */
			for (j = 0; j < ptsseg; j++) {
				if (j == 0) {
					/* If we are at the first point, apply the rest segment
					* length */
					if (a < out_max) {
						out[a] = x1;
						out[a + 1] = y1;
					...
					/* For all other points to be interpolated, calculate
					* x and y delta */
					if (a < out_max) {
						out[a] = (int16)(x1 + j * dx);
	...
	/* Check that all points that should be interpolated actually are. This may
	 ...
	 * pts * 2 is always even, 1 point (end point) is reserved to the very end
	 */
	end = (pts * 2) - 2;
	if (a < end) {
		/* Linearly interpolate missing points between last inserted point and end point */
		for (i = a; i < end; i+= 2) {
			out[i] = (out[i - 2] + in[m - 2]) / 2;
			out[i + 1] = (out[i - 1] + in[m - 1]) / 2;
		}
	}
	/* Set the last sample point to the identical corresponding sample point in
	 * the input pattern. The end point and start point are the only points that
	 * are not computed and that are guaranteed to be identical to the input
	 * patterns' corresponding points.
	 */
...
/*
 * Calculates the number of sample points that needs to be interpolated
...
static float32 pts_per_seg(int16 *pat, int32 n, float32 segl, int16 *segment_buf)
{
    ...
    for (i = 2; i < m; i+= 2) {
		...
		cursegl+= restsegl;
		restsegl = 0.0f;
		/* Find how many points that can be interpolated into this segment */
		ps = (int16)((cursegl / segl));
		if (ps == 0) {
			/* No points for this segment, add the current segment length
			* to the rest segment */
			restsegl+= cursegl;
		}
		else {
		...
		/* Force one point into the very first segment */
		if (i == 2 && ps == 0) {
			ps = 1;
		}
...
/*
 *	Get corner points sequence from signal. The element of the output sequence is point and each pair of
 *	points on the adjacent do not locate on a same key. That means if two points are both corners and 
 *	adjacent and locate on a same key, one point will be removed.
 *
 *	Parameters:
 *		filter:			[in] A pointer to the global filter.
 *		signal:			[in] A pointer to input signal.
 *		n:				[in] The number of points in signal.
 *		corner:			[out] A pointer to output array which saves corner points. The array is allocated by caller.
 *		m:				[out] Indicate caller the number of points of output corner sequence.
 */
static void get_corner_from_signal(Filter *aFilter, int16 *signal, int32 n, int16 *corner, int32 *m)
{
...
/*
 *	Calcuate which points in signal are corners. If the point is a corner, set 1; or set 0. The start and end points
 *	are set 1. For example, input siganl is "x1, x2, x3, x4"(xi is a point), and x2 is a corner, then the output is
 *	"1, 1, 0, 1".
 *
 *	Parameters:
 *		signal:			[in] A pointer to input signal.
 *		n:				[in] The number of points in signal.
 *		corner:			[out] A pointer to uint8 array that indicates which points are corners. 
 *							  The array is allocated by caller whose size should be n.
 */
static void find_corner(int16 *signal, int32 n, uint8 *corner)
{
	Point *vect, *next, *current;
	int16 *curX, *curY, *nextX, *nextY;
	int32 i = 0;
	double length;
	
	vect = (Point *)q_malloc( n * sizeof(Point));
	n--;
	curX = signal;
	curY = signal + 1;
	nextX = signal + 2;
 ...
 *
 *	Parameters:
 *		in:				[in] A pointer to first point sequence.
 *		n:				[in] The number of points of first sequence. (n < MAX_CHARS_PER_STRING)
 *		std:			[in] A pointer to second point sequence. This sequence is often a standard model point sequence.
 *		m:				[in] The number of points of second sequence. (m < MAX_CHARS_PER_STRING)
...
static int16 edit_distance(int16 *in, int32 n, int16 *std, int32 m, int16 key_width, int16 key_height)
{
	...
	/* In order to enhance the last point weight, re-calculate mat[n-1][m-1], namely the return value */
 ...
 *
 *	Return the compare distance between two points. The distance's min value is 0, and max value is 3.
 *	This distance is essentially 1-norm. Suppose one point is in the center of a key, if the other point is also in this
 *	key, the return value is 0. 
 *
 *	Parameters:
 *		x1:				[in] X-coordinate of firest point.
 *		y1:				[in] Y-coordinate of firest point.
...
/*
 *	Return the start direction of a signal. The start direction is defined as a vector. The start point of the vector is
 *	first point of the signal, and the end point is a point in signal which is just far enough from the start point. 
 *	The far threshold is provided by caller and is named radius in this function. 
 *
 *	Parameters:
 *		signal:			[in] A pointer to input signal. 
 *		n:				[in] The number of points in signal. 
 *		radius:			[in] The threshold that is used to determine which point is the end point of direction vector.
...
static void dump_trace(EncodingTable *encoding_table, uint8 *trace, int32 n)
{
	register int32 i;
	int32 index;
	char buf[MAX_N_SAMPLE_POINTS * 4 + 1];

	memset(buf, 0, MAX_N_SAMPLE_POINTS * 4 + 1);
	for (i = 0; i < n; i++) {
		index = trace[i];
...
static void log_play(FILE *stream)
{
	...
	if (matched == 1 && strcmp(label, "InputSignal") == 0) {
		...
		if (input_signal != NULL) {
			...
			/* Free it */
			q_free(input_signal->sample_points);
			q_free(input_signal);
			input_signal = NULL;
		}
	}
...
static InputSignal * log_read_input_signal(FILE *stream)
{
	...
	input_signal->sample_points = q_malloc( sizeof(SwiSamplePoint) * sample_point_count);
	input_signal->sample_point_count = sample_point_count;
	matched = 3;
	index = 0;
	while (matched == 3) {
		matched = fscanf(stream, "%d\t%d\t%d\t", &x, &y, &t);
		if (matched == 3) {
			input_signal->sample_points[index].x = (float32)x;
			input_signal->sample_points[index].y = (float32)y;
			input_signal->sample_points[index].t = (float32)t;
			index++;
		}
	}
	return input_signal;
...
static void log_input_signal(FILE *stream, const InputSignal *input_signal)
{
	...
	n = input_signal->sample_point_count;
	fprintf(stream, "InputSignal");
	fprintf(stream, "\t");
	for (i = 0; i < n; i++) {
		x = (int16)input_signal->sample_points[i].x;
		y = (int16)input_signal->sample_points[i].y;
		t = (int16)input_signal->sample_points[i].t;
