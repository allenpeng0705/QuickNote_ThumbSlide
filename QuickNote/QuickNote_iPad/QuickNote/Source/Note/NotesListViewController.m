...
{	
    ...
    CGRect appFrame = CGRectMake(0.0f, 20.0f, 320.0f, 748.0f);
	...
	CGRect footerBarFrame = rectOfFootBarOfPopList();

    appFrame.origin.y = 0;
	appFrame.size.height -= footerBarFrame.size.height;
	...
	tableview.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin|UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleWidth;
...
{
    DBUtil* noteDB = [DBUtil sharedInstance];
    [self.iNoteContentViewCtrl saveNoteWithChecking:YES];
    if (indexPath.row < [noteDB.iNotes count]) {
...
- (void)inspectNote:(Note *)note animated:(BOOL)animated
{
	...
	*/   
	[self.iNoteContentViewCtrl setupNote:YES];
    [self.iNoteListView reloadData];
}
...
{
    [self.iToolBar setFrame:rectOfFootBarOfPopList()];
}
