...
static int hashtable_expand(QiHashtable* aTable)
{
    ...
    newtable = (struct entry **)q_malloc(sizeof(struct entry*) * newsize);
    ...
    if (NULL != newtable) {
        ...
        newtable = (struct entry **) q_realloc(aTable->iTable, newsize * sizeof(struct entry *));
		if (NULL == newtable) {
				 ...
				 void (*aFreeKeyFn)(void* aKey))
{
	...
	table->iTable = (struct entry **)q_malloc(sizeof(struct entry*) * size);
	if (NULL == table->iTable) { 
		q_free(table); 
		return NULL; 
	}
	memset(table->iTable, 0, size * sizeof(struct entry *));
...
int hashtable_insert(QiHashtable* aTable, void* aKey, void* aValue)
{
    ...
    e = (struct entry *)q_malloc(sizeof(struct entry));
    if (NULL == e) { 
		--(aTable->iEntryCount); 
		return 0; 
	}
...
void hashtable_destroy(QiHashtable* aTable, int aFreeKey, int aFreeValue)
{
	unsigned int i;
	struct entry *e, *f;
	struct entry **table = aTable->iTable;
...
int hashtable_iterator_remove(QiHashtableIter* aIter)
{
	...
	// aIter->iEntry is now outside the hashtable
