...
{
    DBUtil* noteDB = [DBUtil sharedInstance];
    int count = [noteDB.iNotes count];
    
    if (count <= [[Util sharedInstance] maxCountOfNotes]) {
        [noteDB createNote];  
...
- (void)sendAllNotes:(id)sender
{
    ...
    [delegate sendMailWithSubject:@"Notes from QuickNote" Body:rawStr];
}

-(void) showAlertForFullVersion
{
    UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Get Full Version" message:@"This feature cannot support by Lite version, please update to full version" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Get Full Version", nil];
    [alertView show];
    [alertView release];
}
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    switch(buttonIndex)
    {
        case 0: // Cancel
        {
            ...
            break;
        case 1: // Go to full version
        {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewSoftware?id=458083369&mt=8"]];
            break;
